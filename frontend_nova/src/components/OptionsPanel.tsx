...
export const OptionsPanel: React.FC<OptionsPanelProps> = ({
    ...
    const loopModes: Array<{ value: 'auto' | 'single' | 'managed'; label: string }> = [
    ...
    const nextContextMode = contextMode === 'v1' ? 'v2' : contextMode === 'v2' ? 'v3' : 'v1';
    ...
    const loadMemories = useCallback(async () => {
        setIsLoading(true);
        try {
            const data = await fetch(withOwnerQuery('/api/memory')).then(r => r.json());
    ...
    const loadContext = useCallback(async () => {
        if (!sessionId) return;
        try {
            const data = await fetch(withOwnerQuery(`/api/sessions/${sessionId}/context`)).then(r => r.json());
    ...
    const searchMemory = async () => {
        ...
        try {
            const data = await fetch('/api/memory/search', {
                ...
                body: JSON.stringify(withOwnerPayload({ query: searchQuery, top_k: 10 })),
            }).then(r => r.json());
    ...
    const createBackup = async () => {
        ...
        try {
            const resp = await fetch('/api/storage/backup', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ ...storageSelection, backup_label: backupLabel }),
            }).then(r => r.json());
    ...
    const resetStorage = async () => {
        const enabledStores = Object.entries(storageSelection).filter(([, enabled]) => enabled).map(([key]) => key);
        if (enabledStores.length === 0) {
            setStorageMessage('Select at least one store.');
            return;
        }
        if (!window.confirm(`Reset selected storage now?\n\n${enabledStores.join(', ')}\n\nThis can wipe persistent memory and session state.`)) {
            return;
        }
        ...
        try {
            ...
            setStorageMessage(`Reset complete${resp.backup?.backup_name ? ` with backup ${resp.backup.backup_name}` : ''}.`);
            await loadStorage();
        } catch (e) {
                ...
                <div className="options-section">
                    <div className="settings-card" style={{ marginBottom: '20px', padding: '12px', background: 'var(--bg-card)', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                        ...
                        <PremiumSelect
                            ...
                            options={{ 'all': ['auto', 'duckduckgo', 'tavily', 'brave', 'searxng', 'exa'] }}
                            onChange={(val) => setCurrentSearchEngine(val)}
                        />
                        <p className="settings-help" style={{ marginTop: '8px', fontSize: '11px', color: 'var(--text-dim)', lineHeight: 1.4 }}>Select the engine used by the web_search tool. Auto tries SearXNG → Brave → Tavily → Exa → Groq. Set API keys in backend .env for key-based providers.</p>
                    </div>
                    ...
                    <div className="settings-card" style={{ marginBottom: '20px', padding: '12px', background: 'var(--surface2)', borderRadius: '8px', border: '1px solid var(--border)' }}>
                        ...
                        <div style={{ marginTop: '12px' }}>
                            <label className="settings-label" style={{ display: 'block', marginBottom: '6px', fontSize: '12px' }}>Execution Loop</label>
                            <div
                                style={{
                                    display: 'grid',
                                    ...
                                    return (
                                        <button
                                            ...
                                            style={{
                                                ...
                                                background: active ? 'rgba(111, 210, 255, 0.12)' : 'var(--bg)',
                            ...
                            <p className="settings-help" style={{ marginTop: '8px', fontSize: '11px', color: 'var(--text-dim)', lineHeight: 1.4 }}>
                                Auto uses the managed controller when planning is enabled. Single runs the actor loop directly. Managed forces plan → act → observe → verify → commit inside one run.
                            </p>
                            <p className="settings-help" style={{ marginTop: '6px', fontSize: '11px', color: 'var(--accent)', lineHeight: 1.4 }}>
                                {effectiveLoopHint}
                            </p>
                        </div>
                        ...
                        >
                            <div>
                                <label className="settings-label" style={{ display: 'block', marginBottom: '6px', fontSize: '12px' }}>Max Tool Calls</label>
                                <input
                                    type="number"
                                    min={1}
                                    max={24}
                                    inputMode="numeric"
                                    value={maxToolCalls}
                                    onChange={(e) => setMaxToolCalls(e.target.value.replace(/[^\d]/g, '').slice(0, 2))}
                                    placeholder="auto"
                                    style={{
                                        width: '100%',
                                        minHeight: '42px',
                            ...
                            <div>
                                <label className="settings-label" style={{ display: 'block', marginBottom: '6px', fontSize: '12px' }}>Max Turns</label>
                                <input
                                    type="number"
                                    min={2}
                                    max={12}
                                    inputMode="numeric"
                                    value={maxTurns}
                                    onChange={(e) => setMaxTurns(e.target.value.replace(/[^\d]/g, '').slice(0, 2))}
                                    placeholder="auto"
                                    style={{
                                        width: '100%',
                                        minHeight: '42px',
                        ...
                        <p className="settings-help" style={{ marginTop: '8px', fontSize: '11px', color: 'var(--text-dim)', lineHeight: 1.4 }}>
                            Leave blank for adaptive limits. Max turns is total model passes in the loop, so values below 2 are not used.
                        </p>
                    </div>
                    ...
                    <div className="settings-card" style={{ marginBottom: '20px', padding: '12px', background: 'var(--bg-card)', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                        ...
                        >
                            {contextMode === 'v1' ? '📋 V1 Linear' : contextMode === 'v2' ? '⚡ V2 Convergent' : '🧠 V3 Hybrid'}
                        </button>
                        <p className="settings-help" style={{ marginTop: '8px', fontSize: '11px', color: 'var(--text-dim)', lineHeight: 1.4 }}>
                            Per-session toggle. V1 stores linear durable memory. V2 ranks convergent modules by active goals. V3 combines both as an experimental hybrid.
                        </p>
                    </div>
                    ...
                    <div className="settings-card" style={{ marginBottom: '20px', padding: '12px', background: 'var(--bg-card)', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                        <label className="settings-label" style={{ display: 'block', marginBottom: '8px', fontSize: '13px', fontWeight: 600 }}>Semantic Embedding Model</label>
                        <PremiumSelect
                            label=""
                            value={embedModel}
                            options={embedModels}
                        ...
                        />
                        <p className="settings-help" style={{ marginTop: '8px', fontSize: '11px', color: 'var(--text-dim)', lineHeight: 1.4 }}>The model used for semantic vector search in memory. Use a provider-prefixed embedding model such as `ollama:...`, `lmstudio:...`, `llamacpp:...`, or `openai:...` so the runtime uses the correct embeddings endpoint.</p>
                    </div>
                    ...
                    <div className="settings-card" style={{ padding: '12px', background: 'var(--bg-card)', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                        <label className="settings-label" style={{ display: 'block', marginBottom: '8px', fontSize: '13px', fontWeight: 600 }}>Storage Admin</label>
                        <p className="settings-help" style={{ marginTop: 0, marginBottom: '10px', fontSize: '11px', color: 'var(--text-dim)', lineHeight: 1.5 }}>
                            Old DBs can absolutely cause stale memory, old sessions, and retrieval pollution. Use this to back up and reset the exact stores Nova is using.
                        </p>
                        ...
                        <div className="storage-grid">
                            {Object.entries(storageSelection).map(([key, enabled]) => {
                                ...
                                return (
                                    <label key={key} className={`storage-card ${enabled ? 'active' : ''}`}>
                                        <input
                                            type="checkbox"
                                            checked={enabled}
                                            onChange={() => updateStoreSelection(key as keyof typeof storageSelection)}
                                        />
                                        <div>
                                            <div className="storage-card-title">{key.replace(/_/g, ' ')}</div>
                                            <div className="storage-card-meta">
                                                {info?.exists ? `${info.kind} · ${(info.size_bytes / 1024).toFixed(1)} KB` : 'missing'}
                                                {typeof info?.records === 'number' ? ` · ${info.records} rows` : ''}
                                            </div>
                                        </div>
                                    </label>
                                );
                        ...
                        <label className="storage-toggle-row">
                            <input type="checkbox" checked={preserveDefaultAgent} onChange={(e) => setPreserveDefaultAgent(e.target.checked)} />
                            <span>Preserve default agent profile</span>
                        </label>

                        <div className="memory-actions" style={{ marginTop: '10px' }}>
                            <button onClick={loadStorage} className="memory-refresh-btn" disabled={storageLoading}>
                                {storageLoading ? '…' : '↺ Refresh'}
                            </button>
                            <button onClick={createBackup} className="memory-refresh-btn" disabled={storageLoading}>
                                💾 Backup
                            </button>
                            <button onClick={resetStorage} className="memory-clear-btn" disabled={storageLoading}>
                                Reset Selected
                            </button>
                        </div>

                        {storageMessage && <div className="storage-status-note">{storageMessage}</div>}
                                ...
                                {storageBackups.map((backup) => (
                                    <div key={backup.path} className="memory-card">
                                        ...
                                        <div className="memory-footer">
                                            ...
                                            <span className="memory-date">{(backup.size_bytes / 1024).toFixed(1)} KB</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
